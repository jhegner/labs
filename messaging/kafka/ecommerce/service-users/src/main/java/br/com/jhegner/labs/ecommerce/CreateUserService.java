...
public class CreateUserService {
    ...
    private void parse(ConsumerRecord<String, Order> record) throws SQLException {
        ...
        if(isNovoUsuario(order.getEmail())) {
            insereNovoUsuario(order.getUserId(), order.getEmail());
        }
    }

    private void insereNovoUsuario(String userId, String email) throws SQLException {

        var sqlInsertStatement = connection.prepareStatement("insert into users (uuid, email) values (?, ?)");

        sqlInsertStatement.setString(1, userId);
        sqlInsertStatement.setString(2, email);
        sqlInsertStatement.execute();

        System.out.println("O usuario foi adicionado: " + userId + " - " + email);
    }
